...
const { resolveTargetCount, formatTargetCount, formatProgress } = require('./src/targetCount');
const { countCurrentBatchTokens, getNextTokenArchiveBatch } = require('./src/tokenArchive');
const { buildTokenDomainStats, formatTokenDomainStatsTable } = require('./src/tokenDomainStats');
const {
    parseStockWatchArgs,
    formatStockWatchInterval,
    logAggregateStockSkip,
    shouldShowOperatorDetails,
    printCountryPriceTable: printStockCountryPriceTable,
    printOperatorOptionTable: printStockOperatorOptionTable,
...
const SMS_MAX_WAIT_MS = 1 * 60 * 1000;
const SMS_MAX_ATTEMPTS = Math.ceil(SMS_MAX_WAIT_MS / SMS_POLL_INTERVAL); // 1 min
...
function getAvailableMailDomains(limit = DOMAIN_TOKEN_LIMIT) {
    const domains = getConfiguredMailDomains();
    if (!Number.isFinite(limit) || limit <= 0) return domains;
    const stats = getAllTokenDomainStats();
    const counts = new Map((stats.rows || []).map(row => [String(row.domain || '').toLowerCase(), row.count || 0]));
    return domains.filter(domain => (counts.get(String(domain || '').toLowerCase()) || 0) < limit);
}

function pickMailDomain() {
    const domains = getAvailableMailDomains();
...
function printCountryPriceTable(rows, title = '[SMS] HeroSMS 最便宜国家 Top 列表') {
    printStockCountryPriceTable(rows, title);
}

function printOperatorOptionTable(rows, country) {
    printStockOperatorOptionTable(rows, country);
}
...
async function resolveRunSmsOperator(phoneCountry, options = {}) {
    const { debug = false, stockOnly = false } = options;
    ...
    try {
        ...
        if (Number.isFinite(aggregateCount) && aggregateCount >= SMS_OPERATOR_SELECTION_THRESHOLD) {
            logAggregateStockSkip({ stockOnly, country: phoneCountry, aggregateCount });
            return {
                operator: '',
                label: '任何运营商',
        ...
        if (stockOnly) {
            console.log(`[SMS] 库存模式仅展示，不选择运营商；当前默认: ${defaultOption.label}`);
            return defaultOption;
        }

        if (config.heroSmsPromptOperatorSelection === false) {
            console.log(`[SMS] 已关闭运营商交互选择，自动使用默认运营商: ${defaultOption.label} (${phoneCountry.name})`);
            return defaultOption;
        }
...
async function refreshStockSnapshot() {
    ...
    console.log(`\n[库存] 刷新时间: ${timestamp}`);
    console.log(`[库存] service=${config.heroSmsService}, TopN=${Math.max(1, Number(config.heroSmsCountryTopN) || 5)}`);
    const selected = await resolveRunPhoneCountry({ stockOnly: true });
    if (selected && shouldShowOperatorDetails({ stockOnly: true })) {
        await resolveRunSmsOperator(selected, { stockOnly: true });
    }
}

async function startStockWatch() {
    if (!config.heroSmsApiKey) {
        console.error('[错误] 未配置 heroSmsApiKey');
        process.exit(1);
    }

    console.log(`[库存] HeroSMS 库存守护模式启动，每 ${formatStockWatchInterval(STOCK_WATCH_OPTIONS.intervalMs)} 刷新一次，Ctrl+C 停止`);
    ...
    while (true) {
        try {
            ...
            console.error(`[库存] 刷新失败: ${error.message}`);
        }
        console.log(`\n========== [库存] 等待 ${formatStockWatchInterval(STOCK_WATCH_OPTIONS.intervalMs)} 后再次刷新... ==========`);
        await new Promise(resolve => setTimeout(resolve, STOCK_WATCH_OPTIONS.intervalMs));
    }
}
...
async function runSingleRegistration() {
    ...
    const createServices = (useProxy) => {
        ...
        const b = new BrowserService(proxy, {
            ...
            browserWindowPosition: config.browserWindowPosition,
            browserWindowSize: config.browserWindowSize,
        });
        ...
        const o = new OAuthService({ proxy: oauthProxy, tokenBatch: CURRENT_TOKEN_BATCH });
        return { b, o };
    };
...
async function checkTokenCount() {
    ...
    const files = fs.readdirSync(TOKEN_OUTPUT_DIR);
    if (CURRENT_TOKEN_BATCH > 0) {
        return countCurrentBatchTokens(files, CURRENT_TOKEN_BATCH);
    }
    return 0;
}

function resolveCurrentTokenBatch() {
    if (!fs.existsSync(TOKEN_OUTPUT_DIR)) return 1;
    return getNextTokenArchiveBatch(fs.readdirSync(TOKEN_OUTPUT_DIR));
}

function prepareCurrentTokenBatch() {
    CURRENT_TOKEN_BATCH = resolveCurrentTokenBatch();
    console.log(`[批次] 当前执行第 ${CURRENT_TOKEN_BATCH} 批，本批 token 文件将使用 ${CURRENT_TOKEN_BATCH}_ 前缀`);
}

function printTokenDomainStats() {
    const files = fs.existsSync(TOKEN_OUTPUT_DIR) ? fs.readdirSync(TOKEN_OUTPUT_DIR) : [];
    const stats = buildTokenDomainStats(files, getConfiguredMailDomains());
    console.log(formatTokenDomainStatsTable(stats));
}

const DOMAIN_TOKEN_LIMIT = Number.isFinite(Number(config.domainTokenLimit))
    ? Number(config.domainTokenLimit)
    : 60;

function getAllTokenDomainStats() {
    const files = fs.existsSync(TOKEN_OUTPUT_DIR) ? fs.readdirSync(TOKEN_OUTPUT_DIR) : [];
    return buildTokenDomainStats(files, getConfiguredMailDomains());
}

function findRemainingTokenDomain(limit = DOMAIN_TOKEN_LIMIT) {
    if (!Number.isFinite(limit) || limit <= 0) return null;
    const stats = getAllTokenDomainStats();
    return stats.rows.find(row => row.count < limit) || null;
}
...
async function startBatch() {
    const targetCount = await resolveTargetCount(args, {
    ...
    console.log(`[启动] Codex 远程注册机（手机号 + Puppeteer 模式），目标: ${formatTargetCount(targetCount)}`);
    ...
    while (true) {
        ...
        const remainingDomain = findRemainingTokenDomain();
        if (!remainingDomain) {
            console.log(`
[完成] 所有配置域名历史 Token 数量均已达上限 (${DOMAIN_TOKEN_LIMIT})，停止注册机。`);
            printTokenDomainStats();
            break;
        }
        if (Number.isFinite(targetCount) && currentCount >= targetCount) {
            console.log(`\n[完成] Token 数量 (${currentCount}) 已达目标 (${targetCount})。`);
            break;
        }

        console.log(`\n${formatProgress(currentCount, targetCount)} | 单域名历史总上限: ${DOMAIN_TOKEN_LIMIT}`);
...
async function main() {
    ...
    if (STOCK_WATCH_OPTIONS.enabled) {
        await startStockWatch();
        return;
    }
