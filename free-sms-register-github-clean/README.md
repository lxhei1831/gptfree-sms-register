...
本压缩包是用于上传 GitHub 的源码清理版，已经移除运行时敏感数据：

- 已移除 `.git/`、`node_modules/`、日志目录内容、token 输出文件。
- 已移除 `accounts.json`、`username.json`、`shibai.json` 等运行产物。
- 已移除真实配置与历史备份配置。
- `config.json` / `config.example.json` / `config.template.jsonc` 均为空值占位，请自行填写。

## 使用前需要填写

1. `heroSmsApiKey`：HeroSMS API Key。
2. `mailBaseUrl`、`mailAdminEmail`、`mailAdminPassword` 或 `mailAdminToken`：邮箱服务信息。
3. `mailDomain` / `mailDomains`：自己的邮箱域名。
4. 如需代理，填写 `proxyHost`、`proxyPort`、`proxyUsername`、`proxyPassword`。

> 注意：请不要把真实 token、邮箱账号密码、代理账号密码、API Key 提交到公开仓库。
...
| `useChrome/chromePath` | 是否使用指定 Chrome，以及 Chrome 可执行文件路径 | 否 |
| `browserWindowPosition` | Chrome 窗口左上角坐标，例如副屏在主屏左侧时可填 `-1080,0` | 否 |
| `browserWindowSize` | Chrome 窗口尺寸，例如 `1000,900` | 否 |

### 浏览器窗口放到副屏

Windows 多屏坐标以主屏左上角为 `0,0`。如果副屏在主屏左边，X 坐标通常是负数。

你的副屏是 1920x1080 竖屏时，Windows 逻辑尺寸通常是 1080x1920。可以先用：

```json
{
  "browserWindowPosition": "-1080,0",
  "browserWindowSize": "1000,900"
}
```

如果窗口没有完全到副屏，可按 Windows 显示缩放微调 X 坐标，例如 `-960,0` 或 `-1200,0`。
...
3. `mailDomain` 是否为可用域名（例如 `example.com`，不要写错）
