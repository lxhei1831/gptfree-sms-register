...
const { getCurrentBatchTokenName } = require('./tokenArchive');
...
class OAuthService {
    constructor(options = {}) {
        ...
        this.tokenBatch = parseInt(options.tokenBatch, 10) || 0;
    ...
    async exchangeTokenAndSave(code, email) {
        try {
            ...
            const safeEmail = String(email || 'unknown')
                .trim()
                .replace(/[\\/:*?"<>|]/g, '_');
            const filename = getCurrentBatchTokenName(`codex-${safeEmail}-free.json`, this.tokenBatch);
            const savedPaths = [];
