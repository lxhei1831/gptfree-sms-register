...
const { buildBrowserLaunchArgs, resolveViewportSize } = require('./browserWindowOptions');
const { logInputValue } = require('./runLogger');
const { PHONE_LOGIN_BUTTON_TEXTS, buttonTextMatches } = require('./buttonTextMatcher');
...
class BrowserService {
    constructor(proxy, browserOptions = {}) {
        ...
        this.browserOptions = {
            ...
            windowPosition: browserOptions.browserWindowPosition || config.browserWindowPosition || '',
            windowSize: browserOptions.browserWindowSize || config.browserWindowSize || '',
        };
    }
    ...
    async launch() {
        const connectOptions = {
            ...
            args: buildBrowserLaunchArgs({
                position: this.browserOptions.windowPosition,
                size: this.browserOptions.windowSize,
            }),
        };
        ...
        const viewport = resolveViewportSize(this.browserOptions.windowSize);
        ...
        console.log(`[Browser] 浏览器已启动 (${viewport.width}x${viewport.height})`);
    }
    ...
    async clickButtonByText(text, timeout = 10000) {
        ...
        while (Date.now() - start < timeout) {
            const clicked = await this.page.evaluate((texts) => {
                ...
                const matches = (el) => {
                    ...
                    return normalizedText && texts.some(t => normalizedText.includes(normalize(t)));
                };

                for (const b of document.querySelectorAll('button, [role="button"], a, input[type="button"], input[type="submit"]')) {
                    if (b.disabled || b.getAttribute('aria-disabled') === 'true' || !isVisible(b) || !matches(b)) continue;
        ...
        await this.logClickableSnapshot(`未找到按钮: ${candidates.join(' / ')}`).catch(() => {});
        throw new Error(`找不到包含"${candidates.join('" / "')}"的按钮`);
    }

    async logClickableSnapshot(reason = '[Browser] 按钮诊断') {
        const snapshot = await this.page.evaluate(() => {
            const nodes = Array.from(document.querySelectorAll('button, [role="button"], a, input[type="button"], input[type="submit"]'));
            const items = nodes.slice(0, 20).map((el, index) => ({
                index,
                tag: el.tagName.toLowerCase(),
                text: (el.innerText || el.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 120),
                ariaLabel: String(el.getAttribute('aria-label') || '').trim().slice(0, 120),
                title: String(el.getAttribute('title') || '').trim().slice(0, 120),
        ...
        console.warn(`[Browser] ${reason}`);
        console.warn(`[Browser] 当前 URL: ${snapshot.url}`);
        console.warn(`[Browser] 页面文字: ${snapshot.bodyText}`);
        console.warn(`[Browser] 可点击元素: ${snapshot.items.map(item => {
            const label = item.ariaLabel ? ` aria="${item.ariaLabel}"` : '';
            const title = item.title ? ` title="${item.title}"` : '';
            const disabled = item.disabled ? ' disabled' : '';
            return `#${item.index} <${item.tag}> "${item.text}"${label}${title}${disabled}`;
        }).join(' | ')}`);
    }
    ...
    async waitForButtonByText(text, timeout = 30000) {
            ...
            try {
                const found = await this.page.evaluate((texts) => {
                    ...
                    for (const b of document.querySelectorAll('button, [role="button"], a, input[type="button"], input[type="submit"]')) {
                        ...
                        const normalizedText = normalize(text);
                        if (normalizedText && texts.some(t => normalizedText.includes(normalize(t)))) return true;
                    }
                    return false;
                }, candidates);
                if (found) return;
        ...
        await this.logClickableSnapshot(`等待按钮超时: ${candidates.join(' / ')}`).catch(() => {});
    ...
    async navigateToSignup() {
        ...
        await this.clickButtonByText(PHONE_LOGIN_BUTTON_TEXTS, 10000);
    ...
    async oauthLoginAndAuthorize(opts) {
        ...
        for (let round = 0; round < 30; round++) {
            ...
            const hasPhoneLogin = pageInfo.btns.some(b => buttonTextMatches(b, PHONE_LOGIN_BUTTON_TEXTS));
            ...
            if (loginMethod !== 'email' && hasPhoneLogin) {
                ...
                await this.clickButtonByText(PHONE_LOGIN_BUTTON_TEXTS);
                await SLEEP(3000);
                lastHandledUrl = url;
                continue;
            }
